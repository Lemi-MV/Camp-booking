...
?>
    ...
    <h4><strong>Napomena: U polje Tip upisuje se ISKLJUCIVO JEDNA OD DVE OPCIJE -> "Satorsko" ili "Za prikolicu" (bez navodnika). </strong>
        ...
        Ukoliko se kreira satorsko mesto u polja <i>StrujaPrikljucak</i> i <i>VodaPrikljucak</i> treba uneti 0, jer ne postoje mesta za satore sa ovim prikljuccima. </h4>
    <br><br>
...
<?php
	...
	{    
		if(isset($_POST['RedniBrojMesta']) && isset($_POST['Tip']) && isset($_POST['Opis']) && isset($_POST['Kapacitet']) && isset($_POST['SatorPrikolica']) && isset($_POST['Rentira']) && isset($_POST['StrujaPrikljucak']) && isset($_POST['VodaPrikljucak']) && isset($_POST['Cena']))
		{ 
		$upit = "Insert into smestaj (RedniBrojMesta, Tip, Opis, Kapacitet, SatorPrikolica, Rentira, StrujaPrikljucak, VodaPrikljucak, Cena) values (".$_POST['RedniBrojMesta'].", '".$_POST['Tip']."', '".$_POST['Opis']."'," .$_POST['Kapacitet'].",".$_POST['SatorPrikolica'].", ".$_POST['Rentira'].", ".$_POST['StrujaPrikljucak'].", ".$_POST['VodaPrikljucak'].", ".$_POST['Cena'].")";
        $rez = $mysqli->query($upit);
        	if(!$rez)
        	{
        		die("<strong><p style='color: black; font-size: 20px;'> Greska!!!</p></strong>".$mysqli->error);
        	}
        	else
        	{ ?>
