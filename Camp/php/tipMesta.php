...
if(isset($_SESSION['Nivo']) && isset($_GET['tip']))
{
    require_once("php/konekcija.php"); 
    	if($_GET['tip'] == "sat")
    	{ 
    		$upit = "SELECT * FROM smestaj WHERE Obrisan = 0 and SatorPrikolica = 0";
            $rez = $mysqli->query($upit); 

            if($_SESSION['Nivo'] < 4)
            { ?>

            <h3>Sva šatorska mesta u kampu</h3><br>
            <a class="btn btn-primary btn-sm active" href="mesta.php?nova">Kreiraj novo mesto</a><br><br><br>
            ...
            <?php
                ...
                { ?>
                    ...
                    <div class="col-md-10">
                    <input readonly class="form-control text-box single-line" id="Kapacitet" name="Kapacitet" type="number" value="<?php echo $red['Kapacitet'] ?>">
                    </div>
                    </div>  
                    ...
                    <div class="col-md-10">
                    <input readonly class="form-control text-box single-line" id="Cena" name="Cena" type="text" value="<?php echo $red['Cena'] ?>">
                    </div>
                    </div>  
...
<?php 
    	...
    	{ 
    		$upit = "SELECT * FROM smestaj WHERE Obrisan = 0 and SatorPrikolica = 1";
            $rez = $mysqli->query($upit); 

            if($_SESSION['Nivo'] < 4)
            {  ?>
                <h3>Sva mesta za prikolice u kampu</h3><br>
                <a class="btn btn-primary btn-sm active" href="mesta.php?nova">Kreiraj novo mesto</a><br><br>
        <?php 
            }  
                while($red=$rez->fetch_assoc())
                { ?>
